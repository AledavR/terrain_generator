...
void SetupTreeShaderLight(Shader* tree_shader, Vector3 light_dir)
{
  ...
  SetShaderValue(*tree_shader, GetShaderLocation(*tree_shader, "lightDirection"),
                 &light_dir, SHADER_UNIFORM_VEC3);
  SetShaderValue(*tree_shader, GetShaderLocation(*tree_shader, "moonDirection"),
                 &moonDir, SHADER_UNIFORM_VEC3);
  SetShaderValue(*tree_shader, GetShaderLocation(*tree_shader, "moonColor"),
                 &moonColor, SHADER_UNIFORM_VEC3);
}
...
{
  // UnloadTexture(tree_model->materials[0].maps[MATERIAL_MAP_DIFFUSE].texture);
  UnloadShader(tree_model->materials[0].shader);
  UnloadModel(*tree_model);
}

// Dibuja árboles con las posiciones validas obtenidas
